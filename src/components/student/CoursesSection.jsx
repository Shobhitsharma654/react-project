...
<div className='grid gap-4 grid-cols-4 px-4 md:px-0 md:my-16 my-10'>
  {allCourses.slice(0,4).map((course , index) =>
     (<CourseCard  key={index} course={course}/>))}
</div>

      <Link to={'/course-list'} onClick={()=> scrollTo(0,0)}  
      className='text-gray-500 border border-gray-500/70 px-10 py-3 rounded'>Show all courses </Link>
    </div>
  )
}
